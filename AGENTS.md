...
## Core Identity
You are an AI coding assistant working on the **Aqua** application, a specialized subdomain of **Rural Utility Cost** (ruralutilitycost.com).
...
3. **SEO & Mobile First**: All new pages must include the `<SEO>` component with accurate title, description, and keywords. All layouts must be responsive down to 320px. Use `<AdSensePlaceholder>` where appropriate.
...
## Documentation
Please refer to `CALCULATOR-STANDARD.md` whenever generating a new calculator or modifying an existing one.
